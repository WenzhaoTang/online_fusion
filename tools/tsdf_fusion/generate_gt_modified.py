...
def parse_args():
    ...
    parser.add_argument("--data_path", metavar="DIR",
                        help="path to raw dataset", default='/mnt/raid/scanpb/ScanNet_demo')
    ...
    # clip files for multi processes
    parser.add_argument('--start_idx', type=int, default=0, help='# start_idx of clipped list')
    parser.add_argument('--end_idx', type=int, default=200, help='# end idx of clipped list')
...
def split_list_clip(_list, start_idx, end_idx):
    ret = []
    if end_idx > len(_list):
        end_idx = len(_list)
    for idx in range(start_idx, end_idx):
        ret.append(_list[idx])
    return ret
...
if __name__ == "__main__":
    ...
    clip_files = split_list_clip(files, args.start_idx, args.end_idx)

    print('clip files from %d to %d' % (args.start_idx, args.end_idx))

    process_with_single_worker(args, clip_files)
