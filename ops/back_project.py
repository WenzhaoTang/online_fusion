...
def back_project(coords, origin, voxel_size, feats, KRcam, view_centers, addition_aggregation):
    '''
    ...
    if addition_aggregation:
        feature_volume_all = torch.zeros(n_views, coords.shape[0], c + 5, dtype=feats.dtype).cuda()
    else:
        feature_volume_all = torch.zeros(coords.shape[0], c + 1, dtype=feats.dtype).cuda()
    ...
    for batch in range(bs):
        ...
        view_centers_batch = view_centers[:, batch]
        ...
        nV = rs_grid.shape[-1]
        rs_grid_homo = torch.cat([rs_grid, torch.ones([n_views, 1, nV]).cuda()], dim=1)

        # Project grid
        im_p = proj_batch @ rs_grid_homo
        ...
        if addition_aggregation:
            ...
            im_z_norm[im_z <= 0] = 0
            features = torch.cat([features, im_z_norm.unsqueeze(-1)], dim=2)

            # concat pixel validity
            features = torch.cat([features, mask.unsqueeze(-1)], -1)

            # concat viewing rays
            view_centers_batch = view_centers_batch.unsqueeze(-1).expand(-1, -1, nV)
            features = torch.cat([features,
                                  ((rs_grid - view_centers_batch) / (rs_grid - view_centers_batch).norm(dim=1, p=2).unsqueeze(1).expand(-1, 3, -1)).permute(0, 2, 1)],
                                 -1)

            if not features.dtype == feature_volume_all.dtype:
