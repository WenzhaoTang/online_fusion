...
class IntrinsicsPoseToProjection(object):
    ...
    def __call__(self, data):
        ...
        proj_matrices = []
        # add camera centers to calculate viewing ray
        view_centers = []
        for intrinsics, extrinsics in zip(data['intrinsics'], data['extrinsics']):
            ...
            for i in range(3):
                ...
                view_cam_centers.append(-torch.transpose(extrinsics[:3, :3],0, 1) @ extrinsics[:3, 3])
            view_proj_matrics = torch.stack(view_proj_matrics)
            view_cam_centers = torch.stack(view_cam_centers)
            proj_matrices.append(view_proj_matrics)
            view_centers.append(view_cam_centers)
        data['proj_matrices'] = torch.stack(proj_matrices)
        data['view_centers'] = torch.stack(view_centers)
        data.pop('intrinsics')
        data.pop('extrinsics')
        return data
