...
def parse_args():
    ...
    parser.add_argument("--data_path", metavar="DIR",
                        help="path to dataset", default='/mnt/raid/scanpb/ScanNet_Source/public/v2/scans_test')
    parser.add_argument("--gt_path", metavar="DIR",
                        help="path to raw dataset", default='/mnt/raid/scanpb/ScanNet_Source/public/v2/scans_test')
...
def process(scene, total_scenes_index, total_scenes_count):
    ...
    if os.path.exists(os.path.join(save_path, '%s_metrics.json' % scene.replace('/', '-'))):
        print(scene + " metric json already evaluate")
        rslt_file = os.path.join(save_path, '%s_metrics.json' % scene.replace('/', '-'))
        with open(rslt_file) as f:
            metrics = json.load(f)
