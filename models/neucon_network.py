...
from .transformer import TransformerAggregation
...
class NeuConNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.AGGREGATION_ON:
            # input features include additional channels
            ch_in = [80 * alpha + 5, 96 + 40 * alpha + 2 + 5, 48 + 24 * alpha + 2 + 5, 24 + 24 + 2 + 5]
        else:
            ch_in = [80 * alpha + 1, 96 + 40 * alpha + 2 + 1, 48 + 24 * alpha + 2 + 1, 24 + 24 + 2 + 1]

        aggregate_ch_in = [80 * alpha + 5, 40 * alpha + 5, 24 * alpha + 5]
        channels = [96, 48, 24]

        print("Feature aggregation using " + self.cfg.AGGREGATION.MODE if self.cfg.AGGREGATION_ON else "Feature aggregation using summation")

        # feature aggregation
        if self.cfg.AGGREGATION_ON:
            assert self.cfg.AGGREGATION.MODE in ["transformer", "attentional"]
        ...
        for i in range(len(cfg.THRESHOLDS)):
            ...
            if self.cfg.AGGREGATION_ON:
                if self.cfg.AGGREGATION.MODE == "transformer":
                    self.aggregation.append(
                        TransformerAggregation(self.cfg.AGGREGATION.TRANSFORMER, dim=aggregate_ch_in[i])
                    )
                elif self.cfg.AGGREGATION.MODE == "attentional":
                    self.aggregation.append(
                        AttentionalAggregation(dim=aggregate_ch_in[i])
                    )
    ...
    def forward(self, features, inputs, outputs):
        ...
        for i in range(self.cfg.N_LAYER):
            ...
            view_centers = inputs['view_centers'][:, :, scale].permute(1, 0, 2).contiguous()
            volume, count = back_project(up_coords, inputs['vol_origin_partial'], self.cfg.VOXEL_SIZE, feats,
                                         KRcam, view_centers, self.cfg.AGGREGATION_ON)
            grid_mask = count > 1
            ...
            if self.cfg.AGGREGATION_ON:
                ...
                volume = self.aggregation[i](volume)
            ...
            if self.cfg.FP16_ON:
                r_coords = up_coords.detach().clone().half()
            else:
